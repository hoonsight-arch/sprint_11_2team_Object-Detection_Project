"""
YOLO26 학습 스크립트 (앙상블용 두 번째 YOLO 모델)

train_yolo.py(YOLO11)와 동일한 구조. dataset.yaml을 공유해 클래스 인덱스를 맞춘다.
기본 모델: yolo11x.pt  (앙상블에서 yolo11m과 상호보완하도록 더 큰 사이즈 권장)

실행:
    python train_yolo26.py
    python train_yolo26.py --model yolo11x.pt --epochs 60 --batch 4 --imgsz 1280
    python train_yolo26.py --model yolo11l.pt --epochs 80 --batch 8 --imgsz 1280 \
        --mixup 0.1 --copy_paste 0.1 --degrees 10 --flipud 0.5

세션이 죽어도 이어서 학습하려면:
    python train_yolo26.py ... --backup_dir /content/drive/.../outputs/yolo/yolo26 --backup_period 10
    python train_yolo26.py ... --backup_dir <같은 경로> --resume
"""
import argparse
import shutil
...
def parse_args():
    ...
    parser.add_argument("--model",      default="yolo11x.pt",
                        help="모델 크기: yolo11n/s/m/l/x.pt (기본: yolo11x — 앙상블 다양성용)")
    ...
    parser.add_argument("--name",       default="yolo26")
    # 재개 / 백업 옵션
